...
async def calculate_qty(usd_amount: float) -> float:
    ...
    try:
        ...
        # ИСПРАВЛЕНИЕ: Используем кэш exchange.markets после load_markets(), т.к. fetch_market не работает
        market = exchange.markets.get(SYMBOL)
        
        if not market:
            # Это должно произойти, только если load_markets не был вызван или символ неверный
            raise ValueError(f"Рынок {SYMBOL} не найден в кэше. Убедитесь, что load_markets() вызван и символ верен.")
        ...
        # ПРИМЕЧАНИЕ: Для некоторых монет минимальный размер может быть меньше 1.
        # Это запасная мера, лучше полагаться на market['limits']['amount']['min'].
        # Но для простоты:
        if quantity < 1:
            # Проверим минимальное количество, если оно доступно
            min_qty = market.get('limits', {}).get('amount', {}).get('min', 0.0001)
            if quantity < min_qty:
                 quantity = min_qty
                 logger.warning(f"Расчетное количество меньше минимума ({min_qty}). Установлено в {min_qty}.")
...
async def open_position(signal: str, amount_usd=None):
    ...
    try:
        ...
        # Теперь баланс должен быть корректным, и эта проверка будет работать правильно.
...
async def startup_event():
    ...
    try:
        ...
        # НОВОЕ ИСПРАВЛЕНИЕ: Загрузка всех рынков для обхода ошибки 'fetch_market'
        logger.info("Загрузка данных о рынках (обход ошибки fetch_market)...")
