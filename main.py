# main.py — ОКОНЧАТЕЛЬНАЯ ВЕРСИЯ 30.11.2025 — РАБОТАЕТ У ВСЕХ
import os
import time
import hmac
import hashlib
...
def make_signature(params: Dict) -> str:
    # ←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←
    # ЭТО САМАЯ ПРАВИЛЬНАЯ ПОДПИСЬ ДЛЯ BINANCE 2025 ГОДА
    query = "&".join(f"{k}={urllib.parse.quote_plus(str(v))}" for k, v in sorted(params.items()))
...
async def open_long(sym: str):
    symbol = sym.upper().replace("/", "") + ("USDT" if not sym.upper().endswith("USDT") else "")

    if symbol in active:
        await tg(f"<b>{symbol} уже открыт</b>")
        return

    # Cross + плечо — теперь с правильной подписью
...
@app.get("/")
async def root(): return HTMLResponse("<h1>OZ BOT — РАБОТАЕТ</h1>")
...
async def webhook(request: Request):
    ...
    if sig == "LONG":   asyncio.create_task(open_long(s))
    if sig == "CLOSE":  asyncio.create_task(close(s))
    return {"ok": True}
